...
Both publishing jobs explicitly check out using `GH_PAT` and push to `origin`. Never paste credentials into commands, remote URLs, reports or Git. Generation jobs have read-only repository permission; health has issue-write permission but no content-write permission. The PAT's actual access still depends on the secret's scope.

**Verify Publishing Identity** checks the token owner, repository write capability and Git transport dry run. Validate Publication runs for documentation-only PRs as well as code/data changes. Code and feed validation are active, but no new branch rule, reviewer requirement, bypass or Vercel promotion gate has been imposed. Those need an agreed human/editorial versus routine-bot PR policy and the correct platform access. A dry run verifies authentication, not hypothetical future branch rules. Require another smoke test after any policy change.
