...
- `.github/workflows/generate-weekly.yml` prepares the upcoming Friday–Thursday window on Thursday at 03:05 Eastern, with safe catch-up attempts at 09:05, 15:05 and 21:05. It retains all existing dates and reviewed overlaps, so a completed window makes no generation calls. An exhausted API credit balance fails with an explicit Actions error and still requires the operator to restore billing. Daily and weekly generation use `gpt-5.6-terra` for balanced editorial quality and cost, with `gpt-5.4-mini` as a distinct fallback.
...
The operator-approved sequence is **Codex generates the complete month → operator reviews and approves → Codex validates, appends and pushes to GitHub → Vercel deploys the feeds → Codex verifies**. No unattended monthly saints publication is authorized. Daily/weekly production remains automated. Framer page/component changes are a separate reviewed publish operation, not a routine requirement for adding a saints month.
...
The manual monthly generator is a calendar scaffold, not a replacement for editorial review: newly scraped records can have blank profiles and need reviewed Catholic biographical/theological paragraphs. It writes requested-month drafts under `drafts/saints-YYYY-MM.json`, never to `public/saint.json`. It never replaces existing records, including incomplete ones. To improve an existing date, edit it explicitly as part of the review process. Keep the paragraph focused on the saint's life, teachings, Scripture, and Catholic principles, without personal, professional, or financial metaphors. Distinguish later tradition from documented history.
...
The separate Framer component, installation steps, cache/refresh behavior and operator-review release checklist are documented in [framer/README.md](framer/README.md). Git pushes do **not** install or publish Framer code. The operator-approved Archives repair was published and live-tested on 2026-08-30 (Framer version `3a6227de5`) using the production search index. Staging requires a project upgrade and remains disabled; Publish currently goes directly to production. No paid upgrade was made. Routine daily data publication maintains the index without a new Framer publish.
