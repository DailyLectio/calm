...
- `.github/workflows/generate-weekly.yml` prepares the upcoming Friday–Thursday window on Thursday at 03:05 Eastern, with safe catch-up attempts at 09:05, 15:05 and 21:05. It retains all existing dates and reviewed overlaps, so a completed window makes no generation calls. An exhausted API credit balance fails with an explicit Actions error and still requires the operator to restore billing. Both generators retain their existing `gpt-5-mini` model settings.
